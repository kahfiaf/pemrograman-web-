...
new_logic = '''        // 4. Use html2pdf for silent PDF generation
        ...
        // Pass htmlContent string directly, let html2pdf handle the detached iframe!
        window.html2pdf().set(opt).from(htmlContent).save();'''

# Also add width: 800px to the CSS
css_old = "#pdf-report-container{font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:10pt;color:#1a1a1a;background:#fff;padding:25px}"
css_new = "#pdf-report-container{width: 800px; min-width: 800px; max-width: 800px; margin: 0 auto; font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:10pt;color:#1a1a1a;background:#fff;padding:25px}"
...
if old_logic in text:
    ...
    with open('static/app_v3.js', 'w', encoding='utf-8') as f:
        f.write(text)
    print("Successfully reverted to string and added width 800px!")
else:
