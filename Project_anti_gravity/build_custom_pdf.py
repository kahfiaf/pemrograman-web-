    ...
    try {
        ...
        new Chart(document.getElementById('c31'), {
            ...
            data: {
                ...
                datasets: [{ data: qis.map(q => q.finalScore), backgroundColor: entries.map((_, i) => getCol(i)) }]
            },
        ...
        new Chart(document.getElementById('c32'), {
            ...
            data: {
                ...
                datasets: [{ data: Object.values(acts), backgroundColor: ['#94a3b8', '#f59e0b', '#3b82f6', '#10b981'] }]
            },
        ...
        const dimAvgs = dims.map(d => {
            let sum = 0;
            qis.forEach(q => { sum += (q.criteria.find(c=>c.label===d)||{val:0}).val; });
        ...
        new Chart(document.getElementById('c33'), {
            ...
            data: {
                labels: dims,
                datasets: [{ data: dimAvgs, backgroundColor: ['#60a5fa', '#34d399', '#a78bfa', '#fbbf24', '#2dd4bf'] }]
            },
        ...
        new Chart(document.getElementById('c42'), {
            type: 'line',
            data: { labels: histDates, datasets: [{ label: 'Akurasi (%)', data: accs, borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.1)', fill: true, tension: 0.4 }] },
        ...
        new Chart(document.getElementById('c43'), {
            type: 'line',
            data: { labels: histDates, datasets: [{ label: 'Error Rate (%)', data: errs, borderColor: '#d97706', tension: 0.4 }] },
            options: { maintainAspectRatio: false }
        });
        ...
        new Chart(document.getElementById('c44'), {
            type: 'bar',
            data: { labels: histDates, datasets: [{ label: 'Throughput (req/s)', data: thrpts, backgroundColor: '#93c5fd' }] },
            options: { maintainAspectRatio: false }
        });

        // 4.5 Line Latency
        new Chart(document.getElementById('c45'), {
            type: 'line',
            data: { labels: histDates, datasets: [{ label: 'Latency (ms)', data: lats, borderColor: '#111827', tension: 0.4 }] },
            options: { maintainAspectRatio: false }
        });

        // 4.6 Bar Status
        let stPerlu = 0, stBaik = 0;
        errs.forEach(e => { if(e > 0.8) stPerlu++; else stBaik++; });
        new Chart(document.getElementById('c46'), {
            type: 'bar',
            data: { labels: ['Perlu Perhatian', 'Baik'], datasets: [{ data: [stPerlu, stBaik], backgroundColor: ['#374151', '#374151'] }] },
        ...
        const makeEnvChart = (id, lbl, col, base, v) => {
            ...
            new Chart(document.getElementById(id), {
                type: 'line',
                data: { labels: histDates, datasets: [{ label: lbl, data: data, borderColor: col, backgroundColor: col+'22', fill: true, tension: 0.4 }] },
        ...
        const htmlContent = `
<div id="pdf-container" style="width: 800px; max-width: 800px; margin: 0 auto; font-family: Arial, sans-serif; font-size: 9pt; color: #333; background: #fff; padding: 20px;">
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    .header { display: flex; justify-content: space-between; font-size: 8pt; border-bottom: 1px solid #ddd; padding-bottom: 5px; margin-bottom: 15px; }
    .sec-title { font-size: 11pt; font-weight: bold; color: #1e3a8a; border-left: 3px solid #1e3a8a; padding-left: 8px; margin: 20px 0 15px 0; text-transform: uppercase; }
    .grid { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 15px; }
    .card { flex: 1; min-width: 48%; border: 1px solid #cbd5e1; border-radius: 4px; padding: 10px; }
    .card-full { width: 100%; border: 1px solid #cbd5e1; border-radius: 4px; padding: 10px; margin-bottom: 10px; }
    .c-title { font-size: 9pt; font-weight: bold; margin-bottom: 10px; }
    ...
    .page-break { page-break-before: always; height: 1px; margin: 20px 0; }
  </style>
  ...
  <div class="sec-title">5. ENVIRONMENT DASHBOARD</div>
  <div class="grid">
...
if replace_match:
    ...
    with open('static/app_v3.js', 'w', encoding='utf-8') as f:
        f.write(text)
    print("Custom Chart PDF logic written successfully!")
else:
